...
var encodeBody = function(server, route, msgBody) {
    // encode use protobuf
  if(!!server.protobuf && !!server.protobuf.getProtos().server[route] && !msgBody.error) {
    msgBody = server.protobuf.encode(route, msgBody);
  } else if(!!server.decodeIO_protobuf && !!server.decodeIO_protobuf.check(Constants.RESERVED.SERVER, route) && !msgBody.error) {
    var buffer = server.decodeIO_protobuf.encode(route, msgBody);
    var buffer2 = new Buffer(buffer.length + 1); 
    buffer2.writeUInt8(MAGIC_NUMBER, 0);
