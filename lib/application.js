...
Application.use = function(plugin, opts) {
  ...
  if(!plugin.rpcs) {
    ...
    if (!fs.existsSync(plugin.rpcs)) {
      logger.error('fail to find rpcs, find path: %s', plugin.events);
      return;
    }

    fs.readdirSync(plugin.rpcs).forEach(function(filename) {
      if (!/\.js$/.test(filename)) {
        return;
      }
      var absolutePath = path.join(plugin.rpcs, filename);
      if(!fs.existsSync(absolutePath)) {
        logger.error('rpcs %s not exist at %s', filename, absolutePath);
      } else {
        var appRpcConfig = self.get('commonRpcConfig');
        var serverType = self.getServerType();
        if (appRpcConfig == null) {
          appRpcConfig = {};
        }
        if (appRpcConfig[serverType] == null) {
          appRpcConfig[serverType] = [];
        }
        appRpcConfig[serverType].push(path.relative(absolutePath, self.getBase()));
        self.set('commonRpcConfig', appRpcConfig);
      }
    });
  }
};
