...
var getRemotePaths = function(app) {
  ...
  if (services) {
    services = services[serverType];
    if (services) {
      services.forEach(function(service) {
        var commonPath = pathUtil.getUserCommonPath(app.getBase(), service);
        if (commonPath)
          console.log(commonPath);
          paths.push(pathUtil.remotePathRecord('common', serverType, commonPath));
      });
    }
  }
  return paths;
