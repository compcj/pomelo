...
var getProxyRecords = function(app, sinfo) {
  ...
  // patch here
  // user common rpc service path record

  var services = app.get('commonRpcConfig');
  if (services) {
    services = services[sinfo.serverType];
    if (services) {
      services.forEach(function(service) {
        record = pathUtil.getUserCommonPath(appBase, service);
        if (record) {
          records.push(pathUtil.remotePathRecord('common', sinfo.serverType, record));
        }
      });
    }
  }
